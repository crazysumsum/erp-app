...
```json
{
  "accessTokenHash": "<Authorization 那枚 token 的 SHA-256，base64url；沒有 token 時為空字串>",
...
`accessTokenHash` 是同一條理由再推一步，也就是 **RFC 9449（DPoP）的 `ath`**：少了它，簽章不證明這台設備簽的是**配這一枚 token** 的那個請求。攻擊者只要拿得到一份已簽名的續期請求（MITM，或是含 header 的存取紀錄），就能換上同一台設備的另一枚 token 送出去。雜湊而不是放 token 本身，是因為簽章輸入會進日誌與錯誤路徑，而 token 是憑證。登入沒有 token，所以是空字串。

> ⚠️ **這是破壞性的協定變更。** 舊版前端簽出來的輸入少一個欄位，新版後端一定驗不過。伺服器一上線，任何還開著舊 JS 的分頁會在下一次背景續期時被登出——重新載入頁面再登入即可，設備已審批、不需要重審。刻意不做「兩種都收」的相容期：那段期間 `ath` 提供不了任何保護，而且相容碼日後要有人記得回來收掉。
