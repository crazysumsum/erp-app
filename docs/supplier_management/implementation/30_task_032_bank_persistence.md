...
同一個 reviewer 帶住 REV-033 嘅 context 覆驗，判 **APPROVED**（head `dda9a40`）。

佢**冇**靠睇 diff 收貨，而係逐個重跑返揾到問題嗰個探測：

| 原本嘅 finding | 覆驗方法 | 結果 |
| --- | --- | --- |
| H-1 正規化 | 同一個 16 字元探測 | **0 個繞得過**（原本 10 個），另加 128,992 個碼位掃描確認正規化係 idempotent |
| H-2 相容性檢查 | 重起返嗰八張歪表 | **8/8 被拒**（原本 8/8 收咗） |
| H-3 寫入方邊界 | 重跑嗰三個存活變異 | **三個全部紅** |

佢亦都確認咗我對佢自己 M-1 建議嘅更正：byte-length 前綴**修唔到**個 AAD 碰撞，
因為 UTF-8 將每個孤兒代理碼位 map 成同一串三個 byte，兩邊長度都係 3。UTF-16LE 先啱。

### 兩個新 M

**M-1 —— 第九類歪表，我漏咗。** `normalizeExpression` 個 `toLowerCase()` 連引號入面
嘅字面值都轉埋，所以一個寫住 `status = 'ACTIVE'` 嘅生成運算式比較起上嚟等於文件嗰條。
但 `status` 係 `ascii_bin` —— `'ACTIVE'` 永遠對唔到欄位個預設值，slot 永遠 NULL，而
UNIQUE 唔比較 NULL：G1 嗰個「無限個預設」又開返。Reviewer 喺真 MySQL 上面插到五行同時
生效嘅預設。已修，連變異一齊驗過。

**M-2 —— 路由咗俾 Product Owner（DEF-021），冇喺度自己決定。** 正規化器靜靜哋剝走唔
允許嘅字元而唔係拒絕，所以三個唔同嘅輸入會變成同一個儲存帳號（reviewer 示範咗 `ÅB12345`、
`ÄB12345`、`B12345` 全部存成 `B12345` 兼共用一個 blind index），而 NFKC 喺白名單之前行，
所以 46 個碼位會注入純數字（`½` → `12`）。

點解唔喺度決定：HD-028 答咗**保留邊啲字元**，冇被問過**其餘嗰啲點算**。而個分別係真
嘅——排版字元（空格、連字號）一定要剝走，嗰個就係 H-1 白名單嘅全部意義；但 `Å` 同 `½`
唔係排版，佢哋係內容。要分得開就要另一條規則，而嗰條規則同 HD-028 一樣改唔返轉頭。
觸發輸入本身喺 IBAN 同本地格式之下已經無效，而 T33 之前冇 caller，所以係 LOW、唔阻擋。

### 順手收埋

L-1：遮罩嘅 fail-safe 方向錯咗 —— 一行壞資料反而漏得更多（五個字元零粒星）。一個遮罩
函式 fail safe 係遮**多啲**。個測試名同個斷言之前講緊兩件唔同嘅事，正正係呢個模組一路
捉緊嘅形狀。L-2：兩條非唯一索引之前只驗過個名。

### 留低嘅

L-3（`VIRTUAL` 收咗，而設計寫 `STORED`）、L-4（`CHECK` 約束冇讀）、L-6（解密嘅 catch
而家將設定錯誤報成資料完整性錯誤，輪替時會指錯方向俾 operator）。全部 reviewer 標明
唔阻擋。

Trigger 缺口 reviewer 接受咗，但佢自己**核實過**個權限限制（`erp_user` 有
`ALL PRIVILEGES ON erp_dev` 但只有 `USAGE ON *.*`，`log_bin = 1`，`CREATE TRIGGER` 回
ERROR 1419），並且量度咗個缺口 —— 拆走成個檢查，338/338 照綠。佢提咗一個做法值得喺 T33
收：個檢查唯一嘅輸入就係嗰張 trigger 清單，所以借現有嘅 `{ table }` 接口收一個注入嘅
清單，就可以喺冇 DDL 權限之下真係測到嗰條拋錯路徑。
