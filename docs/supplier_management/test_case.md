...
| 設計來源 | docs/supplier_management/design_spec.md 0.2 Draft |
...
- Approval、Supplier Settings、Payment Terms 及職責分離。
...
| --- | --- | --- | --- | --- | --- |
| RQ-01 Migration、catalog、schema及啟動基線 | P0 | MIG-001～MIG-007、OPS-004 | NOT RUN | — | 真MySQL、半套用、seed、Item依賴及fail-closed |
...
| RQ-06 Approval、Settings及Payment Terms | P0 | APR-001～APR-013 | NOT RUN | — | policy snapshot、SoD、stale、重派、並發及typed settings |
...
### 6.1 Migration、Schema、Catalog及啟動基線
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MIG-001 | P0 | NFR-009；全新DB可部署 | Migration/DB | 空白隔離MySQL；已配置fake Bank keys | 0010～0023 migrations | 依runner順序執行全部migration並啟動server | 全部成功；ledger、schema與設計一致；startup self-check健康 | migration輸出、schema hash、health log | — | NOT RUN |
...
| MIG-004 | P1 | FR-SET-003、SEC-001～SEC-007；seed正確 | Migration/DB | 空DB | 六permissions、system-admin、HKD、settings id=1 | Migration後查catalog與seed；重跑 | 六權限及break-glass關聯各一份；HKD Active；approval預設OFF；無未知extra | SQL快照、catalogue startup log | — | NOT RUN |
...
| MIG-006 | P1 | NFR-009；upgrade與forward-only相容 | Deployment/DB | 具舊核心資料的0009 DB及備份 | 現有users/roles/audit | 套用Supplier migrations；驗證舊功能；演練server/client回滾 | 舊資料不變；新schema可讀；不drop/down；回滾邊界符合runbook | 前後SQL、回歸輸出、rollback記錄 | — | NOT RUN |
...
| STATE-007 | P0 | BR-032；提交時重新驗證狀態 | Integration/Concurrency | U-PO先載入S-ACTIVE | lookup後把Supplier suspend/block | 以舊畫面提交新purchase | 提交端再次assertUsable並拒絕；不產生新交易；歷史可讀 | lookup、submit response、DB | — | NOT RUN |
...
### 6.6 Approval、Supplier Settings與Payment Terms
...
| AUTH-002 | P0 | SEC-002～007、BR-022；六權限矩陣 | API/Security | U-VIEW/U-MGMT/U-APR/U-BANK-R/U-BANK-W/U-SET | 所有read/write/actions | 每角色逐route呼叫 | 僅規格組合成功；mgmt不含approval/Bank/settings；bank write需三權限；approval需view+approval | matrix responses、authorization logs | — | NOT RUN |
...
| OPS-006 | P0 | NFR-009；全模組backup/restore | Recovery/DB | 有Core/approval/setting/Bank/import/audit資料 | DB、files、key rings | 備份，隔離restore並執行smoke | 主資料/關係/audit/job/files及keys一致恢復；Bank可受權reveal；缺key時不宣稱成功 | backup manifest、restore SQL/log、smoke evidence | — | NOT RUN |
...
5. Currency除HKD及Payment Term初始資料仍待Finance確認；catalog案例先驗證設計中已確定的HKD seed與通用規則。
