# Fulfillment & Shipping Management 開發執行計劃

## 0. 文件資訊

| 項目 | 內容 |
| --- | --- |
| 依據 | `docs/fulfillment_shipping_management/requirement.md`、`docs/fulfillment_shipping_management/design_spec.md` |
| 產生日期 | 2026-09-09 |
| 任務狀態 | 待人工 review／批准後執行；本文件不代表已開始功能開發 |
...
- [ ] 涉及DB的Phase在明確標記的專用MySQL 5.7測試庫執行fresh／upgrade migration、integration、concurrency及rollback／recovery案例。
...
**Files likely touched：** `docs/fulfillment_shipping_management/tasks.md`及P0 PR描述；如無規格碰撞，不修改production code。
...
Task實作時須把每項新增測試case ID回填到本表或Phase evidence；不得只以章節範圍聲稱覆蓋。`test_case.md`產出後，保留本文件Task ID並建立AC ↔ Test Case ↔ Task ↔ Commit四向追溯。
...
任何上述能力成為正式需求時，必須先更新`requirement.md`、`design_spec.md`、provider contracts、狀態機、Archive eligibility及驗收案例，再重新拆分Tasks；不得直接塞進現有Phase。
