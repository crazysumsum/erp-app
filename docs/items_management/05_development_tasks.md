...
- Definition of Done/status: verification-only adapter, repo-bound trust policy, detached Ed25519 attestation, fail-closed manifest contract and tests are implemented. On 2026-09-21 an isolated restored-copy run passed all 23 checks with RTO 176 ms and RPO 66,000 ms. ERP Product Owner (Sam) approved `APR-025`, explicitly waiving signer independence; the adapter preserves `PASS_WITH_OWNER_WAIVER` while emitting harness status `NOT_RUN`, so this cannot become an ordinary `TC-016` PASS. Independent review and all required PR #121 CI gates passed; PR #121 merged as `513a0bf8100b60384354a576db1172fac59eb60e`. `DONE_WITH_OWNER_WAIVER`.
