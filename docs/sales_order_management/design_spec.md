...
| 建立日期 | 2026-09-08 |
| 依據 | `docs/sales_order_management/requirement.md` |
...
Sales只擁有SO與Reservation mapping；Fulfillment上線後沿用以下正式contract，不能直接寫Sales tables：
...
`applyFulfillmentResultInTransaction()`只接受`SHIPMENT_CONFIRMED`或`SHIPMENT_REVERSED`兩類server allowlist command。前者以Inventory成功結果把Reserved移至Fulfilled；後者必須引用原Shipment Issue及Inventory已恢復的原Reservation，把相同數量由Fulfilled移回Reserved。Sales以provider result覆蓋Reservation mapping projection，不接受caller自報的新結餘。

Fulfillment transaction必須先鎖Sales Order／Lines／Reservation mappings，再鎖Fulfillment aggregate，之後呼叫Inventory batch Issue／Reversal，最後以同一transaction更新Sales quantity及狀態。不得先持有Inventory lock再反向鎖Sales。

`getOrderArchiveStatus()`回`CLOSED／OPEN／UNKNOWN`及safe reason codes；provider unavailable固定為`UNKNOWN`。符合歸檔條件時，Sales Archive Job作唯一協調者，在搬移Sales aggregate的同一transaction呼叫`archiveOrderInTransaction()`搬移及校驗該SO全部Fulfillment／Shipment資料。

Sales在withdraw／cancel／close remaining已鎖Order／Lines後，必須先呼叫`assertSalesLifecycleAllowedInTransaction()`；任何DRAFT、PICKING、PICKED、SHIPPING或REVERSING Fulfillment／Shipment及active Reservation Claim都回`OPEN_FULFILLMENT_EXISTS`，Sales不得繼續release Inventory Reservation。Fulfillment provider只按Sales已鎖owner查詢／加鎖，不反向取得更早的lock。
...
- `COMPLETED`、`CLOSED`、`CANCELLED`是一般業務上的finalized states；未歸檔且沒有不可逆下游事項時，前兩者仍可由具專門權限的Fulfillment Shipment Reversal原子重開。`CANCELLED`不存在已出貨數量，不接受此轉換。
...
- Shipment Confirm只把Reserved移到Fulfilled；Shipment Reversal只把該Shipment的Fulfilled移回原Reservation的Reserved。Close／Cancel把Reserved／Backorder移到Cancelled。
...
- Shipment Reversal後保留既有`cancelled_base_quantity`；若仍有Fulfilled則狀態為`PARTIALLY_FULFILLED`，否則為`CONFIRMED`。被恢復的Reserved重新可供Fulfillment Queue使用，歷史Close／Complete記錄不可改寫。
...
AND every configured downstream open-matter provider returns CLOSED
```

Provider unavailable回`UNKNOWN`，不得當作`CLOSED`。Fulfillment除了回open-matter狀態，亦是同一archive transaction的必要participant；Archive Service在候選掃描及搬移transaction內各檢查一次。
...
每張成功搬移的SO保存一筆跨模組校驗結果；它是Sales Archive Job判斷unknown outcome及restore完整性的正式證據。

| Column | Type | Null／Default | 說明 |
|---|---|---|---|
| `id` | BIGINT UNSIGNED | PK／AI | Manifest ID。 |
| `archive_batch_id` | BIGINT UNSIGNED | NOT NULL／FK batches RESTRICT | 所屬Batch。 |
| `sales_order_id` | BIGINT UNSIGNED | NOT NULL | 原SO ID；不對Active或Archive單邊建FK。 |
| `sales_order_number` | VARCHAR(20) ASCII | NOT NULL | Safe routing／support。 |
| `sales_table_counts`,`sales_hash_summary` | JSON | NOT NULL | Sales逐table counts／canonical hashes。 |
| `fulfillment_table_counts`,`fulfillment_hash_summary` | JSON | NOT NULL | Fulfillment participant結果；未部署前為固定空object。 |
| `participant_versions` | JSON | NOT NULL | Required provider／archive contract versions。 |
| `archived_at` | BIGINT UNSIGNED | NOT NULL | 同一transaction完成時間。 |

`UNIQUE(sales_order_id)`、`INDEX(archive_batch_id,sales_order_id,id)`、`UNIQUE(sales_order_number)`。Manifest與兩邊Archive rows在同一transaction insert；不提供update／delete API，hash不同不得覆蓋。
...
BEGIN
  ...
  4. FulfillmentArchiveParticipant鎖該SO的Fulfillment／Shipment roots並重驗CLOSED
  5. SELECT Sales children + audit；計row counts、money totals、canonical hashes
  6. INSERT Sales archive header/children/history/audit
  7. Participant INSERT Fulfillment archive aggregate並校驗counts/hashes
  8. 從兩組archive tables讀回counts/hashes；不一致立即rollback
  9. 更新external key/conversion及Fulfillment routing hints為archived
 10. Participant先刪Fulfillment active children／roots，再刪Sales active rows
 11. 更新batch counters及跨模組manifest
COMMIT
```
...
6. Export jobs、Archive batch、`sales_archive_order_manifests`及所有Archive Tables；Fulfillment上線時以additive migration擴充participant hash欄位／contract version。
...
- 呼叫Fulfillment lifecycle guard；存在active工作或provider UNKNOWN時fail closed，不先釋放Reservation。
...
`cancel()`：Draft直接terminal；Confirmed零fulfilled時先通過Fulfillment lifecycle guard，再release outstanding reservations、把Reserved＋Backorder移到Cancelled，Backorder entry `CANCELLED`，status `CANCELLED`。

`closeRemaining()`：只限`PARTIALLY_FULFILLED`；先通過Fulfillment lifecycle guard，再釋放所有outstanding reservations，把Reserved＋Backorder移到Cancelled，保留Fulfilled，status `CLOSED`。所有操作全有或全無；Inventory result缺一筆便rollback。

`applyFulfillmentResultInTransaction(..., { action: "SHIPMENT_REVERSED" })`：

- 只接受Fulfillment已驗證的未歸檔Shipment、原Issue refs及Inventory reservation restore results；逐行集合、reservation ID與quantity必須完全相等。
- 在相同transaction把`fulfilled_base_quantity`減少、對應mapping的`consumed_base_quantity`減少及`outstanding_base_quantity`增加；不得改Ordered、Backorder或既有Cancelled。
- 原狀態`COMPLETED／PARTIALLY_FULFILLED／CLOSED`均按更新後數量重算：Fulfilled大於0為`PARTIALLY_FULFILLED`，否則為`CONFIRMED`。
- Append`FULFILLMENT_REVERSE` history／audit並遞增Order與Line versions；相同event replay回原結果，不重複減量。
...
- Fulfillment上線後既是required open-matter provider，也是required archive participant；只回`CLOSED`但沒有participant contract視為配置錯誤並skip。
...
- Sales與Fulfillment共用同一`archive_batch_id`及order-level manifest；任一邊copy／hash／delete失敗均rollback，不能出現父SO與Shipment分處Active／Archive。
...
- Shipment Reversal將Fulfilled移回同一Reservation的Reserved；保留Cancelled，不能令任何quantity為負。
...
- Withdraw／cancel／close在Fulfillment lifecycle guard回OPEN或UNKNOWN時不得先release Reservation，並回穩定錯誤。
...
- Fulfillment Reversal由`COMPLETED／PARTIALLY_FULFILLED／CLOSED`按剩餘Fulfilled重算；CLOSED既有Cancelled不變，history append而非改寫。
- Reversal provider result的line／reservation／quantity集合不完全一致時整體rollback。
