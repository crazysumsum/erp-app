...
本節定義業務資料及關係，不指定實體資料庫表、欄位型別或索引；詳細 Database Table Design 留待 `design_spec.md`。
...
現有 Inventory Reservation 的原子契約不得由 Sales 改成隱式部分成功。Sales 可先按可承諾數量決定本次正式 Reservation Request 數量，未請求部分形成 Backorder；Inventory 對每個正式 Request 仍應是全數成功或失敗。高併發下的重新計算、重試及一致性方案必須在 `design_spec.md` 明確定義並以整合測試驗證。
...
- 支援專案正式支援的桌面瀏覽器；詳細 Browser Matrix 由 `design_spec.md` 及測試計劃定義。
...
本節只定義能力成果，實際工程任務由後續 `tasks.md` 拆分。
...
### 18.3 必須在 `design_spec.md` 前關閉的規格門檻
...
- 後續 `design_spec.md` 可由每個 Capability 獨立導出前端、API、資料庫、整合、權限及 Unit／Integration Test 設計。

### 19.3 後續文件

需求獲確認後，依序產出：

1. `docs/sales_order_management/design_spec.md`：包括詳細前後端架構、API、Database Table Design、狀態與一致性、歸檔技術設計及測試策略。
2. `docs/sales_order_management/tasks.md`：按可獨立交付及測試的 Phase／Task 拆分。
3. `docs/sales_order_management/test_case.md`：以業務流程分類的 UAT 及系統驗收案例。
