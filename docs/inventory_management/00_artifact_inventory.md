...
- Current topic branch: `codex/inventory-p0-foundation`; product implementation was paused before code changes when the database baseline changed.
- Pre-alignment module state: all four narrative inputs are tracked and clean at the recovery point.
- Product source is verification evidence only in the current `DESIGN_AND_PLAN` state; no product-code write is authorized.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `requirement.md` | Primary business-requirement narrative; original Draft now superseded by canonical planning documents | `b97e38187a779403d63210e59a7ec4a8b8d8d9bb2206f96585d2e98c330fc08f`; Git recovery point above | `01_requirement_spec.md` | Complete scope, 125 legacy FRs, 45 BRs, 14 SECs, 14 NFRs, 50 ACs and original DEC-001–020; current baseline adds DEC-021–027 | `MERGE_INTO_CANONICAL`, then `DELETE_AFTER_MERGE` | Replace active references with canonical names | Sam approved the 0.4 MySQL Server 26.7.0 planning baseline on 2026-09-23 |
| `design_spec.md` | Primary system-design narrative; original Draft now superseded by canonical planning documents | `6bc44d5bbf0a1aa066ac3d6c6ffa61e8fc6ecf6fe8a4377f4c4db5ef669d498a`; Git recovery point above | `03_design_spec.md` | Architecture, data model, APIs, security, UI, operations, test design and delivery sequence | `MERGE_INTO_CANONICAL`, then `DELETE_AFTER_MERGE` | Replace active references with canonical names | Sam approved exact MySQL Server 26.7.0 in the updated 0.4 design baseline |
| `tasks.md` | Primary development-plan narrative; original Draft now superseded by canonical planning documents | `76afe464a4382053e2ac6f44dff71a1cdf21bfe0dcdfd2da7a3e8fa4b52059e6`; Git recovery point above | `05_development_tasks.md` | Six Phase boundaries, detailed tasks, gates, dependencies, risks and rollout controls | `MERGE_INTO_CANONICAL`, then `DELETE_AFTER_MERGE` | Replace active references with canonical names | Sam approved the updated P0→P5 plan; resuming IMPLEMENT remains a separate decision |
...
Each legacy narrative is moved as one complete body into its canonical destination. Alignment metadata and formal Harness 2.0 definitions are added around that body; no legacy section, table, diagram, decision, risk, pending item or sign-off field is intentionally discarded. Later in-body edits record the 2026-09-14 approved `DEC-014`–`DEC-026` decisions and the 2026-09-23 approved `DEC-027` exact MySQL Server 26.7.0 compatibility baseline; historical approvals remain recoverable and are not silently reused.
