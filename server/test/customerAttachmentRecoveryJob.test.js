...
test("Customer attachment recovery job is bounded, cluster-scoped and safe when the capability is disabled", async () => {
  ...
  assert.deepEqual(await job.run(), { processed: 0, activated: 0, failed: 0, lastId: 0, stageProcessed: 0, stageCleaned: 0, stageFailed: 0, abandonedOperations: 0, deleteProcessed: 0, deleteCompleted: 0, deleteFailed: 0 });
