  ...
  <div>
    ...
    <main class="q-pa-md" style="max-width: 1000px">
      ...
      <template v-else-if="customer">
        ...
        </q-list></q-tab-panel><q-tab-panel name="addresses"><CustomerAddressPanel :customer-id="customer.id" :addresses="customer.addresses" :can-manage="canManage && customer.status !== 'archived'" @refresh="load" /></q-tab-panel><q-tab-panel name="contacts"><CustomerContactPanel :customer-id="customer.id" :contacts="customer.contacts" :can-manage="canManage && customer.status !== 'archived'" @refresh="load" /></q-tab-panel><q-tab-panel name="identifiers"><CustomerIdentifierPanel :customer-id="customer.id" :identifiers="customer.identifiers" :can-manage="canManage && customer.status !== 'archived'" @refresh="load" /></q-tab-panel><q-tab-panel name="credit"><CustomerCreditPanel :customer-id="customer.id" :policy="customer.credit" :can-manage="canManage && customer.status !== 'archived'" @refresh="load" /></q-tab-panel><q-tab-panel name="banks"><CustomerBankPanel :customer-id="customer.id" :can-manage="canManageBank && customer.status !== 'archived'" :can-reveal="canRevealBank" /></q-tab-panel><q-tab-panel name="attachments"><CustomerAttachmentPanel :customer-id="customer.id" :customer-status="customer.status" :can-manage-general="canManage && customer.status !== 'archived'" :can-view-sensitive="canRevealBank" :can-manage-sensitive="canManageBank && customer.status !== 'archived'" /></q-tab-panel></q-tab-panels>
