...
async function load() { loading.value = true; error.value = null; try { customer.value = await customerService.getById(Number(route.params.id)); loadForm(customer.value); } catch (loadError) { error.value = loadError; } finally { loading.value = false; } }
